						...
						<tbody>
							...
							<c:forEach var="vetour" items="${vetours}">
								...
								<tr>
									...
									<td>${vetour.giaCoc}</td>
									<td scope="row">${vetour.loaiVe.ten}</td>
										...
										<a
										href="suavetour/${vetour.id}.htm"><button
												class="btn btn-primary">chỉnh sửa</button> </a> 

										<button
											onclick="if(confirm('bạn có chắc chắn muốn xóa nhân viên này không ?')){location.href='xoavetour/${vetour.id}.htm'}"
...
">
			    ...
			    <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
			        <a class="page-link" href="../dsvetour/${idBK }.htm?currentPage=${currentPage - 1}">Trước</a>
			    </li>
