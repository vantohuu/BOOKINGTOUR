...
public class start {
	 ...
	 * session.getAttribute("TaiKhoan"); model.addAttribute("taiKhoan",taiKhoan);
	 * return"admin/doimatkhau"; }
	 * 
	 * @RequestMapping(value="updatematkhau", method = RequestMethod.POST) public
	 * String updatematkhau(@ModelAttribute("taiKhoan") TaiKhoan
	 * taiKhoan,@ModelAttribute("PASSWORDCU") String passwordCu,ModelMap
	 * model,BindingResult errors) {
	 * 
	 * 
	 * boolean kt=true; if (passwordCu.trim().length() == 0) {
	 * 
	 * errors.rejectValue("PASSWORD", "taikhoan",
	 * "Vui lòng nhập password !");kt=false; }
	 * 
	 * 
	 * if (taiKhoan.getPASSWORD().trim().length() == 0) {
	 * errors.rejectValue("PASSWORD", "taiKhoan",
	 * "Vui lòng nhập password !");kt=false; }
	 * 
	 * kt=false; } else if (!taiKhoan.getPASSWORD().trim().equals(passwordCu)) {
	 * errors.rejectValue("PASSWORD", "taikhoan",
	 * "Password cũ và mới không khớp!");kt=false; } else if
	 * (!searchTaiKhoan(taiKhoan.getMANV()).getPASSWORD().trim().equals(passwordCu.
	 * trim())) { errors.rejectValue("PASSWORD", "taikhoan",
	 * "Password cũ không đúng!");kt=false; }
	 * 
	 * 
	 * if(kt==true) {
	 * 
	 ...
	 * session.beginTransaction(); try { session.update(taiKhoan); t.commit();
	 * model.addAttribute("message", 1); } catch (Exception e) { t.rollback();
	 * model.addAttribute("message", 2);
	 * 
