				...
				<div class="container form-dang-nhap">
					...
					<table class="table">
						...
						<c:set var="counter" value="${offset+1 }" />
							<c:forEach var="ctphongluutru" items="${ctphongluutrus}">

								<tr>
										<th scope="row">${counter}</th>
									
									<td>${ctphongluutru.phong.ten} - ${ctphongluutru.phong.noiLuuTru1.tenNLT} </td>
									<td>${ctphongluutru.ngayDen} - ${ctphongluutru.ngayDi} </td>
												<td>	<c:if test="${ctphongluutru.fullNgay ==1}">
												Ở theo ngày
														</c:if>
														<c:if test="${ctphongluutru.fullNgay ==0}">
												Ở theo giờ
														</c:if>
								</td>
													...
													<div class="modal-body" style="width: 500px;">
														...
														<div class="row">
															<div class="col">
																...
																<label for="recipient-name"
																	class="col-form-label">${ctphongluutru.phong.soGiuong}</label>
															</div>
														</div>
														<div class="row">
															<div class="col">
																...
																<label for="recipient-name"
																	class="col-form-label">${ctphongluutru.phong.sLNMax}</label>
															</div>
														</div>
														<div class="row">
															<div class="col">
																...
																<label for="recipient-name"
																	class="col-form-label">${ctphongluutru.phong.hangPhong}</label>
															</div>
														</div>
								 ...
								 --%>
										...
										<button
											onclick="if(confirm('bạn có chắc chắn muốn xóa nhân viên này không ?')){location.href='../xoadatphong/${ctphongluutru.id}.htm'}"
...
">
			    ...
			    <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
			        <a class="page-link" href="../dsdatphong/${idBK }.htm?currentPage=${currentPage - 1}">Trước</a>
			    </li>
			    <c:forEach begin="0" end="${totalPages - 1}" var="i">
			        <li class="page-item ${currentPage == i ? 'active' : ''}">
			            <a class="page-link" href="../dsdatphong/${idBK }.htm?currentPage=${i}">${i + 1}</a>
			        </li>
			    </c:forEach>
			    <li class="page-item ${currentPage == totalPages - 1 ? 'disabled' : ''}">
			        <a class="page-link" href="../dsdatphong/${idBK }.htm?currentPage=${currentPage + 1}">Sau</a>
			    </li>
			</ul>
			</div>
				</div>
